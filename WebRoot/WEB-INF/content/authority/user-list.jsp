...
<script type="text/javascript">
	$(document).ready(function() {
		$("#gridTable").jqGrid({
			url: "${ctx}/authority/userList.action",
	   		datatype: "json",
	   		mtype: "post",
	   		colNames:['id','用户帐号',"电话号码","身份证","录入时间","状态", '操作'],
	    	colModel: [ 
				{name:"id", index:"id", sortable:false, align:"center", hidden:true},
				{name:"userAccount", index:"papel", width:150, sortable:false, align:"center"}, 
				{name:"userPhone", index:"papel", width:150, sortable:false, align:"center"}, 
				{name:"userIDCard", index:"papel", width:150, sortable:false, align:"center"}, 
				{name:"inserttime", index:"papel", width:150, sortable:false, align:"center"}, 
				{name:"userStatus", index:"papel", width:150, sortable:false, align:"center"}, 
	...
	function freshGrid(){
		$("#gridTable").jqGrid("setGridParam",{url:"${ctx}/authority/userList.action"}).trigger("reloadGrid");
	}
	...
	function doUpdate(id){
		...
		diag.Title = "编辑";
		diag.URL = "${ctx}/authority/userInput.action?id="+id;
		diag.show();
	}
	...
	function doDelete(id){
		...
		if(isSure==true){
			...
			dc.add("id",id);
			AjaxServer.sendRequest("${ctx}/authority/deleteUser.action",dc.toJSON(),function(response){
				if(response==1){
					alert("删除成功!");
					$("#gridTable").jqGrid("setGridParam",{url:"${ctx}/authority/userList.action"}).trigger("reloadGrid");
				}
...
<body>
	<div class="right01"><img src="${ctx}/images/04.gif" /> 权限管理 &gt; <span>查询用户</span></div>
	<div class="main">
   <div class="bar">
      <label style="margin-left:30px;">用户名称:</label><input type="text" class="inputClass" id="qNum" name="qNum" />
