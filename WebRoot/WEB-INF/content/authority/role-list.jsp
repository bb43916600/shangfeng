...
<script type="text/javascript">
	$(document).ready(function() {
		$("#gridTable").jqGrid({
			url: "${ctx }/authority/roleList.action",
	   		datatype: "json",
	   		mtype: "post",
	   		colNames: ["id","角色名称", "添加用户", "添加菜单","添加权限",'操作'],
	    	colModel: [ 
				{name:"id", index:"id", sortable:false, align:"center", hidden:true},
				{name:"papel", index:"papel", width:150, sortable:false, align:"center"}, 
				{name:"usuario", index:"usuario", width:80, sortable:false, align:"center"},
				{name:"menu", index:"menu", width:80, sortable:false, align:"center"},
				{name:"permisos", index:"permisos", width:80, sortable:false, align:"center"},
				{name:'Operating',index:'Operating', width:120, align:"center"}	
			],
			autowidth: true,
			rowNum: 15,
			rowList: [15,30,50],
	...
	function freshGrid(){
		$("#gridTable").jqGrid("setGridParam",{url:"${ctx }/authority/roleList.action"}).trigger("reloadGrid");
	}
	
	function addpapel(id){
		var diag =new Dialog();
		diag.Width = 700;
		diag.Height = 420;
		diag.parentHelp=window;
		diag.Title = "设置角色";
		diag.URL = "${ctx}/authority/setRole.action?roleid="+id;
		diag.show();
	}
	...
	function addmenu(id){
		...
		diag.Height = 420;
		diag.Title = "添加菜单";
		diag.URL = "${ctx}/authority/addMenuDetail.action?roleid="+id;
		diag.show();
	}
	
	function addpermisos(id){
		var diag =new Dialog();
		diag.Width = 700;
		diag.Height = 420;
		diag.parentHelp=window;
		diag.Title = "添加权限";
		diag.URL = "${ctx}/authority/addOperationDetail.action?roleid="+id;
		diag.show();
	}
	...
	function updateRole(id){
		...
		diag.parentHelp=window;
		diag.Title = "编辑";
		diag.URL = "${ctx}/authority/inputRole.action?id="+id;
		diag.show();
	}
	...
	function deleteRole(id){
		...
		if(isSure==true){
			...
			dc.add("id",id);
			AjaxServer.sendRequest("${ctx}/authority/deleteRole.action",dc.toJSON(),function(response){
				if(response==1){
					alert("删除成功!");
					$("#gridTable").jqGrid("setGridParam",{url:"${ctx }/authority/roleList.action"}).trigger("reloadGrid");
				}
...
<body>
	<div class="right01"><img src="${ctx}/images/04.gif" /> 权限管理 &gt; <span>角色管理</span></div>
	<div class="main">
   <div class="bar">
      <label style="margin-left:30px;">查找:</label><input type="text" class="inputClass" id="qNum" name="qNum" />
