...
<script type="text/javascript">
	$(document).ready(function() {
		$("#gridTable").jqGrid({
			url: "${ctx}/authority/menuList.action",
	   		datatype: "json",
	   		mtype: "post",
	   		colNames:['id','所属栏目','菜单名称','URL', '操作'],
	    	colModel: [ 
				{name:'id',index:'id', width:60,hidden:true},
				{name:'name',index:'name', width:90,align:"center"},
				{name:'submenu',index:'submenu', width:90,align:"center"},
				{name:'url',index:'url', width:180,align:"left"},
	...
	function updateMenu(id){
		...
		diag.Title = "编辑";
		diag.URL = "${ctx}/authority/menuinput.action?id="+id;
		diag.show();
	}
	...
	function deleteMenu(id){
		...
		if(isSure==true){
			...
			dc.add("id",id);
			AjaxServer.sendRequest("${ctx}/authority/deleteMenu.action",dc.toJSON(),function(response){
				if(response==1){
					alert("成功添加!");
					$("#gridTable").jqGrid("setGridParam",{url:"${ctx}/authority/menuList.action"}).trigger("reloadGrid");
				}
				else {
					alert("操作有误!");
				}	
...
<body>
	<div class="right01"><img src="${ctx}/images/04.gif" /> 权限管理 &gt; <span>菜单管理</span></div>
	<div class="main">
   <div class="bar">
      <label style="margin-left:30px;">名称:</label><input type="text" class="inputClass" id="qNum" name="qNum" />
