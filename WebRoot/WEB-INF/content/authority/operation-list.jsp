...
<script type="text/javascript">
	$(document).ready(function() {
		$("#gridTable").jqGrid({
			url: "${ctx}/authority/operationList.action",
	   		datatype: "json",
	   		mtype: "post",
	   		colNames:['id','权限名称', '操作'],
	    	colModel: [ 
	...
	function updateOperation(id){
		...
		diag.Title = "编辑";
		diag.URL = "${ctx}/authority/operation-input.action?id="+id;
		diag.show();
	}
	
	function freshGrid(){
		$("#gridTable").jqGrid("setGridParam",{url:"${ctx}/authority/operationList.action"}).trigger("reloadGrid");
	}
	...
	function deleteOperation(id){
		...
		if(isSure==true){
			...
			dc.add("id",id);
			AjaxServer.sendRequest("${ctx}/authority/deleteOperation.action",dc.toJSON(),function(response){
				if(response==1){
					alert("删除成功!");
					$("#gridTable").jqGrid("setGridParam",{url:"${ctx}/authority/operationList.action"}).trigger("reloadGrid");
				}
...
<body>
	<div class="right01"><img src="${ctx}/images/04.gif" /> 权限管理 &gt; <span>权限管理</span></div>
	<div class="main">
   <div class="bar">
      <label style="margin-left:30px;">权限名称:</label><input type="text" class="inputClass" id="qNum" name="qNum" />
