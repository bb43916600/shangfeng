...
function addObject(){
	...
	$.post("${ctx }/hr/conceptSave.action",dc.toJSON(),function(response){
		if(response==1){
			alert("提交成功!");
			if(id!=0 && id!=""){
...
<body>
	<div class="right01"><img src="${ctx }/images/04.gif" /> 上风实业 &gt; <span>理念设置</span></div>
	<div class="main">
	   <div class="bar"></div>
	   <form id="addObject" method="post">
	   <table width="80%" border="0" cellpadding="0" cellspacing="0" class="table">
	   		<input type="hidden" name="concept.id" id="id" value="${concept.id }">
	      <tr>
	      	<td width="120" align="right">理念标题:</td>
	      	<td><input type="text" class="inputClass" style="width:300px;" id="title" name="concept.title" value="${concept.title }" /></td>
	      </tr>
	      <tr>
	      	<td width="120" align="right">理念详细内容:</td>
	      	<td><textarea  class="xheditor" style="width:100%; height: 200px;" id="content" name="concept.content" >${concept.content }</textarea></td>
	      </tr>
	      <tr>
