...
function addObject(){
	...
	$.post("${ctx }/investment/companyInfoSave.action",dc.toJSON(),function(response){
		if(response==1){
			alert("提交成功!");
			$("#title").attr("value","");
			$("#content").val("");
...
<body>
	<div class="right01"><img src="${ctx }/images/04.gif" /> 上风实业 &gt; <span>公告录入</span></div>
	<div class="main">
	   <div class="bar"></div>
	   <form id="addObject" method="post">
	   <table width="80%" border="0" cellpadding="0" cellspacing="0" class="table">
	   		<input type="hidden" name="companyInfo.id" id="id" value="${companyInfo.id }">
	      <tr>
	      	<td width="120" align="right">标题:</td>
	      	<td><input type="text" class="inputClass" style="width:300px;" id="title" name="companyInfo.title" value="${companyInfo.title }" /></td>
	      </tr>
	      <tr>
	      	<td width="120" align="right">内容:</td>
	      	<td><textarea  class="xheditor" style="width:100%; height: 200px;" id="content" name="companyInfo.content" >${companyInfo.content }</textarea></td>
	      </tr>
	      <tr>
