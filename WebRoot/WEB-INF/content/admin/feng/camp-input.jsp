...
function addObject(){
	...
	$.post("${ctx }/contactus/campSave.action",dc.toJSON(),function(response){
		if(response==1){
			alert("提交成功!");
			$("#title").attr("value","");
			$("#content").val("");
...
<body>
	<div class="right01"><img src="${ctx }/images/04.gif" /> 上风实业 &gt; <span>营地录入</span></div>
	<div class="main">
	   <div class="bar"></div>
	   <form id="addObject" method="post">
	   <table width="80%" border="0" cellpadding="0" cellspacing="0" class="table">
	   		<input type="hidden" name="camp.id" id="id" value="${camp.id }">
	      <tr>
	      	<td width="120" align="right">营地名称:</td>
	      	<td><input type="text" class="inputClass" style="width:300px;" id="title" name="camp.title" value="${camp.title }" /></td>
	      </tr>
	      <tr>
	      	<td width="120" align="right">营地详细介绍:</td>
	      	<td><textarea  class="xheditor" style="width:100%; height: 200px;" id="content" name="camp.content" >${camp.content }</textarea></td>
	      </tr>
	      <tr>
