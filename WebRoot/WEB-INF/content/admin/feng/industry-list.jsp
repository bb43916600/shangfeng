...
<script type="text/javascript">
	$(document).ready(function() {
		$("#gridTable").jqGrid({
			url: "${ctx}/industry/industryList.action?type=${type}",
	   		datatype: "json",
	   		mtype: "post",
	...
	function doUpdate(id){
		...
		diag.Title = "编辑";
		diag.URL = "${ctx}/industry/industryInput.action?id="+id;
		diag.show();
	}
	...
	function doDelete(id){
		...
		if(isSure==true){
			...
			dc.add("id",id);
			AjaxServer.sendRequest("${ctx}/industry/industryDelete.action",dc.toJSON(),function(response){
				if(response==1){
					alert("删除成功!");
					$("#gridTable").jqGrid("setGridParam",{url:"${ctx}/industry/industryList.action?type=${type}"}).trigger("reloadGrid");
				}
				else {
					alert("操作有误!");
				}	
...
<body>
	<div class="right01"><img src="${ctx}/images/04.gif" /> 上风实业 &gt; <span>产业列表</span></div>
	<div class="main">
   <div class="bar">
