...
<script type="text/javascript">
	$(document).ready(function() {
		$("#gridTable").jqGrid({
			url: "${ctx}/about/honorList.action",
	   		datatype: "json",
	   		mtype: "post",
	   		colNames:['id','荣誉名称','详细介绍','录入时间', '操作'],
	    	colModel: [ 
	...
	function doUpdate(id){
		...
		diag.Title = "编辑";
		diag.URL = "${ctx}/about/honorInput.action?id="+id;
		diag.show();
	}
	...
	function doDelete(id){
		...
		if(isSure==true){
			...
			dc.add("id",id);
			AjaxServer.sendRequest("${ctx}/about/honorDelete.action",dc.toJSON(),function(response){
				if(response==1){
					alert("删除成功!");
					$("#gridTable").jqGrid("setGridParam",{url:"${ctx}/about/honorList.action"}).trigger("reloadGrid");
				}
				else {
					alert("操作有误!");
				}	
...
<body>
	<div class="right01"><img src="${ctx}/images/04.gif" /> 上风实业 &gt; <span>荣誉列表</span></div>
	<div class="main">
   <div class="bar">
      <label style="margin-left:30px;">荣誉名称:</label><input type="text" class="inputClass" id="qNum" name="qNum" />
