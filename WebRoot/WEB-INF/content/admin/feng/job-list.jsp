...
<script type="text/javascript">
	...
	function doUpdate(id){
		...
		diag.Title = "编辑";
		diag.URL = "${ctx}/hr/jobInput.action?id="+id;
		diag.show();
	}
	...
	function doDelete(id){
		...
		if(isSure==true){
			...
			dc.add("id",id);
			AjaxServer.sendRequest("${ctx}/hr/jobDelete.action",dc.toJSON(),function(response){
				if(response==1){
					alert("删除成功!");
					$("#gridTable").jqGrid("setGridParam",{url:"${ctx}/hr/jobList.action"}).trigger("reloadGrid");
				}
				else {
					alert("操作有误!");
				}	
