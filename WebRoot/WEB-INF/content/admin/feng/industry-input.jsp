...
function addObject(){
	...
	$.post("${ctx }/industry/industrySave.action",dc.toJSON(),function(response){
		if(response==1){
			alert("提交成功!");
			$("#title").attr("value","");
			$("#content").val("");
...
<body>
	<div class="right01"><img src="${ctx }/images/04.gif" /> 上风实业 &gt; <span>产业录入</span></div>
	<div class="main">
	   <div class="bar"></div>
	   <form id="addObject" method="post">
	   <table width="80%" border="0" cellpadding="0" cellspacing="0" class="table">
	   		<input type="hidden" name="industry.id" id="id" value="${industry.id }">
	   		<input type="hidden" name="industry.type" id="type" value="${industry.type }">
	      <tr>
	      	<td width="120" align="right">标题:</td>
	      	<td><input type="text" class="inputClass" style="width:300px;" id="title" name="industry.title" value="${industry.title }" /></td>
	      </tr>
	      <tr>
	      	<td width="120" align="right">内容:</td>
	      	<td><textarea  class="xheditor" style="width:100%; height: 200px;" id="content" name="industry.content" >${industry.content }</textarea></td>
	      </tr>
	      <tr>
