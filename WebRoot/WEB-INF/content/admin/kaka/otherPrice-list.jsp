...
<script type="text/javascript">
	$(document).ready(function() {
		$("#gridTable").jqGrid({
			url: "${ctx}/otherPrice/objectList.action",
	   		datatype: "json",
	   		mtype: "post",
	   		colNames:['id','繁体名称','英文名称','简体名称','备注(繁体)','备注(英文)','备注(简体)','所属','录入时间', '操作'],
	    	colModel: [ 
				{name:'id',index:'id', width:60,hidden:true},
				{name:'ct_otherName',index:'ct_otherName', width:120,align:"center"},
				{name:'en_otherName',index:'en_otherName', width:120,align:"center"},
				{name:'cn_otherName',index:'cn_otherName', width:120,align:"center"},
				{name:'ct_remark',index:'ct_remark', width:120,align:"center"},
				{name:'en_remark',index:'en_remark', width:120,align:"center"},
				{name:'cn_remark',index:'cn_remark', width:120,align:"center"},
				{name:'parentName',index:'parentName', width:120,align:"center"},
	...
	function doSearch(){
		...
		$("#gridTable").jqGrid("setGridParam",{
			...
			rows: rows, 
	  		postData:{
	  			"otherPrice.${language}_otherName": $("#qNum").val(),
	     	}
		}).trigger("reloadGrid");
	}
	...
	function doUpdate(id){
		...
		diag.Title = "编辑";
		diag.URL = "${ctx}/otherPrice/objectInput.action?id="+id;
		diag.show();
	}
	...
	function doDelete(id){
		...
		if(isSure==true){
			...
			dc.add("id",id);
			AjaxServer.sendRequest("${ctx}/otherPrice/objectDelete.action",dc.toJSON(),function(response){
				if(response==1){
					alert("删除添加!");
					$("#gridTable").jqGrid("setGridParam",{url:"${ctx}/otherPrice/objectList.action"}).trigger("reloadGrid");
				}
				else {
					alert("操作有误!");
				}	
...
<body>
	<div class="right01"><img src="${ctx}/images/04.gif" /> 卡卡叫车系统 &gt; <span>额外需求列表</span></div>
	<div class="main">
   <div class="bar">
      <label style="margin-left:30px;">名称(${language}):</label><input type="text" class="inputClass" id="qNum" name="qNum" />
