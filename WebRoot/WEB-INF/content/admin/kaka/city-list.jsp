...
<script type="text/javascript">
	$(document).ready(function() {
		$("#gridTable").jqGrid({
			url: "${ctx}/city/objectList.action",
	   		datatype: "json",
	   		mtype: "post",
	   		colNames:['id','繁体名称','英文名称','简体名称','所属区域','录入时间', '操作'],
	    	colModel: [ 
				{name:'id',index:'id', width:60,hidden:true},
				{name:'ct_cityName',index:'ct_cityName', width:120,align:"center"},
				{name:'en_cityName',index:'en_cityName', width:120,align:"center"},
				{name:'cn_cityName',index:'cn_cityName', width:120,align:"center"},
				{name:'areaId',index:'areaId', width:120,align:"center"},
	...
	function doSearch(){
		...
		$("#gridTable").jqGrid("setGridParam",{
			...
			rows: rows, 
	  		postData:{
	  			"city.${language}_cityName": $("#qNum").val(),
	  			/* "city.${language}_cityName": $("#qNum").val(), */
	     	}
		}).trigger("reloadGrid");
	}
	...
	function doUpdate(id){
		...
		diag.Title = "编辑";
		diag.URL = "${ctx}/city/objectInput.action?id="+id;
		diag.show();
	}
	...
	function doDelete(id){
		...
		if(isSure==true){
			...
			dc.add("id",id);
			AjaxServer.sendRequest("${ctx}/city/objectDelete.action",dc.toJSON(),function(response){
				if(response==1){
					alert("删除添加!");
					$("#gridTable").jqGrid("setGridParam",{url:"${ctx}/city/objectList.action"}).trigger("reloadGrid");
				}
				else {
					alert("操作有误!");
				}	
	...
	/* function showData() {
		var userData = $("#gridTable").getGridParam("ceshi");
		alert(userData);
	} */
...
<body>
	<div class="right01"><img src="${ctx}/images/04.gif" /> 卡卡叫车系统 &gt; <span>城市列表</span></div>
	<div class="main">
   <div class="bar">
      <label style="margin-left:30px;">名称(${language}):</label><input type="text" class="inputClass" id="qNum" name="qNum" />
      <%-- <label style="margin-left:30px;">区域(${language}):</label>
      ...
      <!-- <input type="text" class="inputClass" id="areaId" name="areaId" /> -->
