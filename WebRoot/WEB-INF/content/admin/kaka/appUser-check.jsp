...
<script type="text/javascript">
	$(document).ready(function() {
		$("#gridTable").jqGrid({
			url: "${ctx}/appUser/checkList.action",
	   		datatype: "json",
	   		mtype: "post",
	   		colNames:['id','车牌号码','手机号码','用户昵称','车类型','出厂日期','状态','等级','录入时间','最后次登陆时间', '操作'],
	    	colModel: [ 
				{name:'id',index:'id', width:60,hidden:true},
				{name:'carNumber',index:'ct_title', width:120,align:"center"},
				{name:'phone',index:'en_title', width:120,align:"center"},
				{name:'userName',index:'cn_title', width:120,align:"center"},
				{name:'carType',index:'ct_content', width:120,align:"center"},
				{name:'factoryDate',index:'en_content', width:120,align:"center"},
				{name:'state',index:'cn_content', width:120,align:"center"},
				{name:'level',index:'inputTime', width:90,align:"center"},
				{name:'inputTime',index:'inputTime', width:90,align:"center"},
				{name:'lastTime',index:'inputTime', width:90,align:"center"},
	...
	function doUpdate(id){
		...
		diag.Title = "编辑";
		diag.URL = "${ctx}/appUser/objectDetail.action?id="+id;
		diag.show();
	}
	...
	function doDelete(id){
		...
		if(isSure==true){
			...
			dc.add("id",id);
			AjaxServer.sendRequest("${ctx}/appUser/updateState.action",dc.toJSON(),function(response){
				if(response==1){
					alert("操作成功!");
					$("#gridTable").jqGrid("setGridParam",{url:"${ctx}/appUser/objectList.action"}).trigger("reloadGrid");
				}
				else {
					alert("操作有误!");
				}	
	...
	function freshGrid(){
		$("#gridTable").jqGrid("setGridParam",{url:"${ctx}/appUser/checkList.action"}).trigger("reloadGrid");
	}
</script>
</head>
<body>
	<div class="right01"><img src="${ctx}/images/04.gif" /> 卡卡叫车系统 &gt; <span>待审核司机</span></div>
	<div class="main">
   <div class="bar">
      <label style="margin-left:30px;">车牌:</label><input type="text" class="inputClass" id="qNum" name="qNum" />
