...
<script type="text/javascript">
	$(document).ready(function() {
		$("#gridTable").jqGrid({
			url: "${ctx}/systemInfo/objectList.action",
	   		datatype: "json",
	   		mtype: "post",
	   		colNames:['id','繁体标题','英文标题','简体标题','繁体内容','英文内容','简体内容','录入时间', '操作'],
	    	colModel: [ 
				{name:'id',index:'id', width:60,hidden:true},
				{name:'ct_title',index:'ct_title', width:120,align:"center"},
				{name:'en_title',index:'en_title', width:120,align:"center"},
				{name:'cn_title',index:'cn_title', width:120,align:"center"},
				{name:'ct_content',index:'ct_content', width:120,align:"center"},
				{name:'en_content',index:'en_content', width:120,align:"center"},
				{name:'cn_content',index:'cn_content', width:120,align:"center"},
	...
	function doSearch(){
		...
		$("#gridTable").jqGrid("setGridParam",{
			...
			rows: rows, 
	  		postData:{
	  			"systemInfo.${language}_title": $("#qNum").val(),
	     	}
		}).trigger("reloadGrid");
	}
	...
	function doUpdate(id){
		...
		diag.Title = "编辑";
		diag.URL = "${ctx}/systemInfo/objectInput.action?id="+id;
		diag.show();
	}
	...
	function doDelete(id){
		...
		if(isSure==true){
			...
			dc.add("id",id);
			AjaxServer.sendRequest("${ctx}/systemInfo/objectDelete.action",dc.toJSON(),function(response){
				if(response==1){
					alert("删除添加!");
					$("#gridTable").jqGrid("setGridParam",{url:"${ctx}/systemInfo/objectList.action"}).trigger("reloadGrid");
				}
				else {
					alert("操作有误!");
				}	
...
<body>
	<div class="right01"><img src="${ctx}/images/04.gif" /> 卡卡叫车系统 &gt; <span>系统公告</span></div>
	<div class="main">
   <div class="bar">
      <label style="margin-left:30px;">标题(${language}):</label><input type="text" class="inputClass" id="qNum" name="qNum" />
