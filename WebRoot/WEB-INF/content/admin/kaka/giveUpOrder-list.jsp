...
<script type="text/javascript">
	$(document).ready(function() {
		$("#gridTable").jqGrid({
			url: "${ctx}/giveUpOrder/objectList.action",
	   		datatype: "json",
	   		mtype: "post",
	   		colNames:['id','订单','用户','录入时间', '操作'],
	    	colModel: [ 
				{name:'id',index:'id', width:60,hidden:true},
				{name:'orderID',index:'orderID', width:120,align:"center"},
				{name:'appUserID',index:'appUserID', width:120,align:"center"},
	...
	function doUpdate(id){
		...
		diag.Title = "编辑";
		diag.URL = "${ctx}/giveUpOrder/objectInput.action?id="+id;
		diag.show();
	}
	...
	function doDelete(id){
		...
		if(isSure==true){
			...
			dc.add("id",id);
			AjaxServer.sendRequest("${ctx}/giveUpOrder/objectDelete.action",dc.toJSON(),function(response){
				if(response==1){
					alert("删除添加!");
					$("#gridTable").jqGrid("setGridParam",{url:"${ctx}/giveUpOrder/objectList.action"}).trigger("reloadGrid");
				}
				else {
					alert("操作有误!");
				}	
...
<body>
	<div class="right01"><img src="${ctx}/images/04.gif" /> 卡卡叫车系统 &gt; <span>弹射订单</span></div>
	<div class="main">
   <div class="bar">
      <label style="margin-left:30px;">用户名:</label><input type="text" class="inputClass" id="qNum" name="qNum" />
