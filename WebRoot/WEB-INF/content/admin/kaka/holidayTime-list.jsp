...
<script type="text/javascript">
	$(document).ready(function() {
		$("#gridTable").jqGrid({
			url: "${ctx}/holidayTime/objectList.action",
	   		datatype: "json",
	   		mtype: "post",
	   		colNames:['id','节假日时间',"备注",'录入时间', '操作'],
	    	colModel: [ 
				{name:'id',index:'id', width:60,hidden:true},
				{name:'settingTime',index:'settingTime', width:120,align:"center"},
	...
	function doSearch(){
		...
		$("#gridTable").jqGrid("setGridParam",{
			...
			rows: rows, 
	  		postData:{
	  			"holidayTime.settingTime": $("#qNum").val()+" 00:00:00",
	     	}
		}).trigger("reloadGrid");
	}
	...
	function doUpdate(id){
		...
		diag.Title = "编辑";
		diag.URL = "${ctx}/holidayTime/objectInput.action?id="+id;
		diag.show();
	}
	...
	function doDelete(id){
		...
		if(isSure==true){
			...
			dc.add("id",id);
			AjaxServer.sendRequest("${ctx}/holidayTime/objectDelete.action",dc.toJSON(),function(response){
				if(response==1){
					alert("删除添加!");
					$("#gridTable").jqGrid("setGridParam",{url:"${ctx}/holidayTime/objectList.action"}).trigger("reloadGrid");
				}
				else {
					alert("操作有误!");
				}	
...
<body>
	<div class="right01"><img src="${ctx}/images/04.gif" /> 卡卡叫车系统 &gt; <span>节假日列表</span></div>
	<div class="main">
   <div class="bar">
