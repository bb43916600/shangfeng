...
<script type="text/javascript">
	$(document).ready(function() {
		$("#gridTable").jqGrid({
			url: "${ctx}/linePrice/objectList.action",
	   		datatype: "json",
	   		mtype: "post",
	   		colNames:['id','开始城市','结束城市','价钱','录入时间', '操作'],
	    	colModel: [ 
				{name:'id',index:'id', width:60,hidden:true},
				{name:'startCityId',index:'startCityId', width:120,align:"center"},
				{name:'endCityId',index:'endCityId', width:120,align:"center"},
	...
	function doSearch(){
		...
		$("#gridTable").jqGrid("setGridParam",{
			...
			rows: rows, 
	  		postData:{
	  			"linePrice.${language}_linePriceName": $("#qNum").val(),
	     	}
		}).trigger("reloadGrid");
	}
	...
	function doUpdate(id){
		...
		diag.Title = "编辑";
		diag.URL = "${ctx}/linePrice/objectInput.action?id="+id;
		diag.show();
	}
	...
	function doDelete(id){
		...
		if(isSure==true){
			...
			dc.add("id",id);
			AjaxServer.sendRequest("${ctx}/linePrice/objectDelete.action",dc.toJSON(),function(response){
				if(response==1){
					alert("删除添加!");
					$("#gridTable").jqGrid("setGridParam",{url:"${ctx}/linePrice/objectList.action"}).trigger("reloadGrid");
				}
				else {
					alert("操作有误!");
				}	
...
<body>
	<div class="right01"><img src="${ctx}/images/04.gif" /> 卡卡叫车系统 &gt; <span>线路列表</span></div>
	<div class="main">
   <div class="bar">
